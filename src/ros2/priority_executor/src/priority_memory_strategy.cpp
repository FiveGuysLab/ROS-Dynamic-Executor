...
template <>
void PriorityMemoryStrategy<>::add_executable_to_queue(
    std::shared_ptr<PriorityExecutable> e) {
...
template <> void PriorityMemoryStrategy<>::clear_handles() {
  ...
  // all_executables_ = std::priority_queue<const PriorityExecutable *,
  // std::vector<const PriorityExecutable *>, PriorityExecutableComparator>();
  // all_executables_.clear();
  // all_executables_ = std::map<const PriorityExecutable *, int,
...
void PriorityMemoryStrategy<>::remove_null_handles(rcl_wait_set_t *wait_set) {
  ...
  // Important to use subscription_handles_.size() instead of wait set's size
  // since there may be more subscriptions in the wait set due to Waitables
  // added to the end. The same logic applies for other entities.
  ...
  subscription_handles_.erase(std::remove(subscription_handles_.begin(),
                                          subscription_handles_.end(), nullptr),
...
template <>
bool PriorityMemoryStrategy<>::collect_entities(
...
template <>
void PriorityMemoryStrategy<>::add_waitable_handle(
...
template <>
bool PriorityMemoryStrategy<>::add_handles_to_wait_set(
    rcl_wait_set_t *wait_set) {
  for (auto subscription : subscription_handles_) {
    if (rcl_wait_set_add_subscription(wait_set, subscription.get(), NULL) !=
        RCL_RET_OK) {
      RCUTILS_LOG_ERROR_NAMED("rclcpp",
                              "Couldn't add subscription to wait set: %s",
  ...
  for (auto client : client_handles_) {
    if (rcl_wait_set_add_client(wait_set, client.get(), NULL) != RCL_RET_OK) {
      RCUTILS_LOG_ERROR_NAMED("rclcpp", "Couldn't add client to wait set: %s",
                              rcl_get_error_string().str);
      return false;
    }
  }
  ...
  for (auto service : service_handles_) {
    if (rcl_wait_set_add_service(wait_set, service.get(), NULL) != RCL_RET_OK) {
      RCUTILS_LOG_ERROR_NAMED("rclcpp", "Couldn't add service to wait set: %s",
                              rcl_get_error_string().str);
      return false;
    }
  }
  ...
  for (auto timer : timer_handles_) {
    if (rcl_wait_set_add_timer(wait_set, timer.get(), NULL) != RCL_RET_OK) {
      RCUTILS_LOG_ERROR_NAMED("rclcpp", "Couldn't add timer to wait set: %s",
                              rcl_get_error_string().str);
      return false;
    }
  }
...
void PriorityMemoryStrategy<>::get_next_executable(
    rclcpp::AnyExecutable &any_exec, const WeakNodeList &weak_nodes) {

  const PriorityExecutable *next_exec = nullptr;
  // std::cout << "all_executables_.size():" << all_executables_.size() <<
  // std::endl; log contents of all_executables_ std::cout << exec->name << ": "
  // << exec->handle << " : " << exec->sched_type << std::endl;
  std::set<rclcpp::CallbackGroup::SharedPtr> skip_groups;
  ...
  for (auto exec : all_executables_) {
    ...
    case SUBSCRIPTION: {
      ...
      if (subscription) {
        ...
        if (skip_groups.find(group) != skip_groups.end()) {
          // group was used at some point during this loop. skip it so we can
        ...
        // std::cout << "Using new priority sub " <<
    ...
    case SERVICE: {
      ...
      if (service) {
        ...
        any_exec.node_base = get_node_by_group(group, weak_nodes);
        // std::cout << "Using new priority service " <<
    ...
    case CLIENT: {
      ...
      if (client) {
        ...
        any_exec.node_base = get_node_by_group(group, weak_nodes);
        // std::cout << "Using new priority client " <<
    ...
    /*         std::ostringstream oss;
            ...
            {
                // if (exec->can_be_run)
                oss << "{\"name\":\"" << exec->name << "\", \"sched_type\":\""
       << exec->sched_type << "\", \"can_be_run\":\"" << exec->can_be_run <<
       "\"},";
            }
    ...
    // \"type\":\"" << type << "\"}"; log_entry(logger_, oss.str());
    return;
  }
  // nothing found
  // std::stringstream oss;
  // oss << "{\"operation\":\"get_next_executable\",
  // \"result\":\"nothing_found\"}"; log_entry(logger_, oss.str());
}
...
void PriorityMemoryStrategy<>::post_execute(rclcpp::AnyExecutable any_exec,
  ...
  if (any_exec.subscription) {
    next_exec =
        get_priority_settings(any_exec.subscription->get_subscription_handle());
  ...
  if (next_exec == nullptr) {
    RCLCPP_WARN(rclcpp::get_logger("rclcpp"),
                "Could not find priority settings for handle");
  }
  ...
  if (next_exec->is_first_in_chain && next_exec->sched_type != DEADLINE) {
    ...
    uint64_t millis = (current_time.tv_sec * (uint64_t)1000) +
    (current_time.tv_nsec / 1000000);

    auto timer = next_exec->timer_handle;
    int64_t time_until_next_call = timer->time_until_trigger().count() /
    1000000;
    // std::cout << "end of chain. time until trigger: " <<
  ...
  if (next_exec->is_last_in_chain && next_exec->sched_type == DEADLINE) {
    ...
    // RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "last, chain id: %d",
    // next_exec->chain_id); for (auto deadline : *next_exec->deadlines)
    // {
    //     RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "deadline: %d", deadline);
    // }
    std::ostringstream oss;
    // oss << "{\"operation\": \"deadline_check\", \"chain_id\": " <<
    // next_exec->chain_id << ", \"deadline\": " <<
    // next_exec->deadlines->front() << ", \"current_time\": " << millis << "}";
    // log_entry(logger_, oss.str());

    if (next_exec->timer_handle == nullptr) {
      std::cout << "tried to use a chain without a timer handle!!!"
                << std::endl;
    }
    auto timer = next_exec->timer_handle;
    if (timer == nullptr) {
      std::cout << "somehow, this timer handle didn't have an associated timer"
                << std::endl;
    }
    // timespec current_time;
    // clock_gettime(CLOCK_MONOTONIC_RAW, &current_time);
    // uint64_t millis = (current_time.tv_sec * (uint64_t)1000) +
    // (current_time.tv_nsec / 1000000); uint64_t this_deadline =
    // next_exec->deadlines->front();
    ...
    int periods_late;
    // if time_diff is negative, we completed on time. add one period to the
    // deadline
    if (time_diff <= 0) {
      periods_late = 0;
    ...
    // if time_diff is positive, we completed late. add one period for each
    // period we were late
    else {
    ...
    // std::chrono::nanoseconds time_until =
    // next_exec->timer_handle->time_until_trigger(); next_deadline = millis +
    // (time_until.count() / 1000000) + next_exec->period; the next deadline is
    // the current time plus the period, skipping periods that have already
    // passed

    // RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "next deadline: %d",
    // next_deadline);
    oss << "{\"operation\": \"next_deadline\", \"chain_id\": "
        << next_exec->chain_id << ", \"deadline\": " << next_deadline
        << ", \"on_time\": " << on_time << ", \"time_diff\": " << time_diff
    ...
    // RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "chain id: %d",
    // next_exec->chain_id); for (auto deadline : *next_exec->deadlines)
    // {
    //     RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "deadline: %d", deadline);
    // }
  ...
  if (next_exec->sched_type == CHAIN_AWARE_PRIORITY ||
    ...
    std::shared_ptr<PriorityExecutable> mut_executable =
        get_priority_settings(next_exec->handle);
...
template <>
void PriorityMemoryStrategy<>::get_next_subscription(
    rclcpp::AnyExecutable &any_exec, const WeakNodeList &weak_nodes) {
...
template <>
void PriorityMemoryStrategy<>::get_next_service(
    rclcpp::AnyExecutable &any_exec, const WeakNodeList &weak_nodes) {
...
template <>
void PriorityMemoryStrategy<>::get_next_client(rclcpp::AnyExecutable &any_exec,
                                               const WeakNodeList &weak_nodes) {
...
template <>
void PriorityMemoryStrategy<>::get_next_timer(rclcpp::AnyExecutable &any_exec,
                                              const WeakNodeList &weak_nodes) {
...
template <>
void PriorityMemoryStrategy<>::get_next_waitable(
    rclcpp::AnyExecutable &any_exec, const WeakNodeList &weak_nodes) {
